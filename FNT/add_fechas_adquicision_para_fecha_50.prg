...
TEXT TO lqry_consultar NOSHOW 
  select * from bienes_patrimoniales.AFECHASPATRIMONIO WHERE VFECHA = '01/01/1950';
...
SCAN
   ...
   TEXT TO lqry_consultar_fecha NOSHOW 
     SELECT TO_CHAR(FECHA_TRAN, 'DD/MM/YYYY') AS  VFECHA_TRAN FROM bienes_patrimoniales.transferenciabienes where 
      fecha_tran <> to_date('1950-01-01', 'yyyy-mm-dd') and iidbien in (SELECT iidbien   FROM bienes_patrimoniales.bienespatrimonio where vcodbarra = ?lc_barra)  order by fecha_tran limit 1
    ENDTEXT
