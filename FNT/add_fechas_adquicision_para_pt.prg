...
TEXT TO lqry_consultar NOSHOW 
    select * from bienes_patrimoniales.AFECHASPATRIMONIO WHERE barras = '301681'
ENDTEXT
...
SCAN
   ...
   TEXT TO lqry_consultar_fecha NOSHOW 
	 SELECT TO_CHAR(FECHA_TRAN, 'DD/MM/YYYY') AS  VFECHA_TRAN FROM bienes_patrimoniales.transferenciabienes where iidbien in 
		(SELECT iidbien   FROM bienes_patrimoniales.bienespatrimonio where vcodbarra = ?lc_barra) order by fecha_tran limit 1
    ENDTEXT
